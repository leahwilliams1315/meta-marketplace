...
interface MultipleSelectorProps {
  ...
  options?: Option[];
  placeholder?: string;
  /** Minimum characters required before showing 'Add new tag' prompt */
  minCharactersForAddNew?: number;
  /** Custom prompt text for adding new tags */
  createNewPrompt?: string;
  /** Callback when user confirms creating a new tag */
...
const MultipleSelector = React.forwardRef<MultipleSelectorRef, MultipleSelectorProps>(
  (
    ...
    // Check if we should show the 'Add new' prompt
    const shouldShowAddNewPrompt = React.useMemo(() => {
      if (!minCharactersForAddNew || !onCreateNew || inputValue.length < minCharactersForAddNew) {
        return false;
      }
    ...
    // Warn if feature is partially configured
    React.useEffect(() => {
      if (minCharactersForAddNew !== undefined && !onCreateNew) {
        console.warn('MultipleSelector: minCharactersForAddNew is set but onCreateNew callback is missing');
      }
    }, []);
    ...
    const handleKeyDown = React.useCallback(
      (e: React.KeyboardEvent<HTMLDivElement>) => {
        ...
        if (input) {
          ...
          if (e.key === 'Enter' && shouldShowAddNewPrompt && onCreateNew) {
            e.preventDefault();
            e.stopPropagation();
    ...
    return (
      ...
      >
            ...
            <CommandPrimitive.Input
              ...
              className={cn(
                'flex-1 bg-transparent outline-none placeholder:text-muted-foreground',
                {
                  ...
                  'text-blue-600 font-medium': shouldShowAddNewPrompt
                },
        ...
        <div className="relative">
          <AnimatePresence>
            {shouldShowAddNewPrompt && (
              <motion.div
                initial={{ opacity: 0, y: 10 }}
                animate={{ opacity: 1, y: 0 }}
                exit={{ opacity: 0, y: 10 }}
                className="absolute -bottom-8 left-0 z-50 w-full px-2 py-1 text-sm text-blue-600 bg-blue-50 rounded-md shadow-sm border border-blue-200"
              >
